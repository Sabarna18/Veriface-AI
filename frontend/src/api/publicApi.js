...
export const publicApi = {
  ...
  fetchTodayAttendance: (classroomId) => {
    return httpPublic(`/attendance/today?classroom_id=${classroomId}`);
  },
  ...
  fetchAllAttendanceForClassroom: (classroomId) => {
    return httpPublic(`/attendance/all?classroom_id=${classroomId}`);
  },
  ...
  getTodayAttendanceForClassroom: (classroomId) => {
    return httpPublic(`/classrooms/${classroomId}/attendance/today`);
  },
  ...
  fetchUser: (userId, classroomId) => {
    return httpPublic(`/users/${userId}?classroom_id=${classroomId}`);
  },
  ...
  getUserFaceImage: (userId, classroomId) => {
    return httpPublic(`/users/${userId}/image?classroom_id=${classroomId}`, {
      responseType: "blob",
    });
  },
  ...
  // POST /recognize/
  // POST /recognize/
  recognizeUser: async (userId, imageFile) => {
    if (!imageFile) {
    ...
    const response = await httpPublic("/recognize/", {
      method: "POST",
    ...
    if (response?.reason === "OUTDATED_EMBEDDING") {
      return {
        ...response,

        message: "Face embedding outdated. Please re-register user.",
      };
    }
